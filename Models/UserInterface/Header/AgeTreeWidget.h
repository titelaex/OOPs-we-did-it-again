...
public:
    ...
    // Set current player index (0 or1)
...
private:
    ...
    // Selective card update: hide a specific picked card without rebuilding
    void hidePickedCard(int nodeIndex);
    ...
    // Store graphics items per node for selective updates
    std::vector<QGraphicsRectItem*> m_cardRects;
    std::vector<QGraphicsTextItem*> m_cardTexts;

    // Selective hiding vectors
    std::vector<int> m_hiddenCardsAges;
    std::vector<int> m_hiddenCardsIndexes;
};
